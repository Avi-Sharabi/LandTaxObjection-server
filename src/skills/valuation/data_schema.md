...
| `cover_facts` | list | `{label, value}` rows of the cover fact table. |
...
- `comp_summary_row`: the navy summary row under the comparables (`ref, date, price, area, zone, rate, comparison` — all strings here). The `ref` value (e.g. `"CPV ADOPTED RANGE"`) spans the # **and** Address columns (colspan=2 in the template); remaining fields map left-to-right to Date → Price → Area m² → Zone → $/m² → CPV Comparison.
...
- `constraints[].status` — use colored spans for CONFIRMED (green), REFUSED (red), UNCONFIRMED (amber)
- `financial_scenarios[].scenario` — use amber spans for current-VG baseline rows
- `cpv.comp_summary_row.price` and `.comparison` — support `<br>` for line breaks
