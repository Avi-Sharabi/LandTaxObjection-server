...
export class AzureEmailService implements OnModuleInit {
  ...
  private async send(message: EmailMessage): Promise<void> {
    ...
    if (result.status !== 'Succeeded') {
      throw new Error(
        `Azure ACS email delivery failed — status=${result.status} id=${result.id}`,
      );
    }
  }
  ...
  async sendDisputeApplication(
    ...
    const urgentPrefix = details?.deadlineLapsedWarning
      ? '⚠ DEADLINE LAPSED — '
      : '';
        ...
        to: [
          {
            ...
            displayName: params.recipientName ?? 'Valuer-General Office',
          },
        ],
      },
      content: {
