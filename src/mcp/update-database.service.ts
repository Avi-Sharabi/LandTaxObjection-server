  ...
  async execute(
    ...
    try {
      const response = await firstValueFrom(
        this.http.post<{ content: { type: string; text?: string }[] }>(
          this.config.getOrThrow<string>('ANTHROPIC_API_URL'),
          {
            model: 'claude-sonnet-4-6',
            max_tokens: 4000,
            thinking: { type: 'enabled', budget_tokens: 2000 },
            system: [{ type: 'text', text: skillContent, cache_control: { type: 'ephemeral' } }],
            messages: [{
              role: 'user',
              content: [
                `${dto.Instructions}`,
          ...
          },
          {
            headers: {
              ...
              'anthropic-beta': 'mcp-client-2025-04-04,prompt-caching-2024-07-31,interleaved-thinking-2025-05-14',
              'Content-Type': 'application/json',
            },
          },
        ),
      );
      const textBlock = response.data.content?.findLast((b) => b.type === 'text');
