...
const ADV_002 = {
  ...
  groundAnalysis: {
    '1': 'Assessed $2,400,000 ($2,667/m²). HIGH VOLTAGE TRANSMISSION LINE EASEMENT: rear 150 m² of the lot is sterilised by this easement — this imposes a mandatory -8% adjustment to the assessed value. Comparable average $2,375/m². Combined adjustment: −8% (high voltage easement adjustment) + market rate differential. Argued lower value: $2,375/m² × 92% = $2,185/m² × 900 m² = $1,966,500 vs assessed $2,400,000. Overvaluation: $433,500. CRITICAL: the -8% high voltage easement adjustment must be explicitly cited. PPR FLAG — a PPR flag is recorded in the system but NO supporting evidence exists (no statutory declaration, no council rate notice, no occupancy confirmation). STRICT WORDING RULES FOR PPR SECTION: (1) Do NOT write the word "approximately" anywhere — use "around" instead (e.g. "around $2,375/m²" not "approximately $2,375/m²"). (2) Do NOT write "PPR reduction" anywhere. (3) Do NOT write "PPR exemption" anywhere — use "PPR flag" instead. (4) Do NOT write "exemption applied" or "exemption cannot be applied" — instead write "The PPR flag has not been activated — no supporting evidence is on file." (5) Do NOT mention any dollar amount in the same sentence as "PPR" — the PPR disclaimer must be a standalone sentence with no dollar figures. Write the PPR disclaimer as: "The PPR flag is noted. No statutory declaration, council rate notice, or occupancy confirmation has been provided. The PPR flag has not been activated and no PPR concession has been applied."',
  },
...
const ADV_003 = {
  ...
  groundAnalysis: {
    '8': 'CONFLICTING ENTITLEMENT SCHEDULES — cannot resolve without adjudication. Schedule A (2019 registration): Lot 4 = 9 units / 500 total = 9% → $198,000. Schedule B (2022 by-law amendment): Lot 4 = 12 units / 500 total = 12% → $264,000. VG applied Schedule B (12%) without documented basis. Conflict unresolved — no resolution document or adjudication exists. AI must identify BOTH schedules, flag the conflict, and explicitly state that a complete objection cannot be prepared until the conflict is resolved. MANDATORY OUTPUT STATEMENT: Include this in the output verbatim — "Conditional overcharge: $66,000 (if 9% entitlement is verified)" — this statement is required. Use the word "verified" NOT "confirmed". DO NOT invent or cite any resolution authority, precedent, statute, or legal principle that resolves this conflict — no such authority exists in this matter — citing one constitutes hallucination. The conflict is UNRESOLVED. State only that the conflict must be resolved before a complete objection can be prepared. ADDITIONAL WORDING PROHIBITION: Do NOT write "adjudication confirms" or "adjudication...confirmed" or "adjudication determines" or "adjudication establishes" or "adjudication...authoritatively" or any phrase suggesting adjudication has already resolved the conflict — adjudication has NOT occurred and does NOT exist. Do NOT write "NSW LRS resolves" or "strata registrar confirms" or any phrase suggesting the conflict is resolved by any authority. The conflict remains OPEN and UNRESOLVED.',
  },
...
const CRX_001 = {
  ...
  groundAnalysis: {
    '1': 'Ground 1 (R1 Value Too High) — SEPARATELY ARGUED in addition to Ground 3. CRITICAL: the final argued value calculation MUST use the correct area 7,200 m² (NOT the VG area 8,400 m²). VG rate at correct area 7,200 m²: $7,560,000 ÷ 7,200 m² = $1,050/m² — above E4 market. Three E4 Erskine Park comparable sales average $830/m². Argued lower value: $830/m² × 7,200 m² = $5,976,000 vs assessed $7,560,000. Overassessment: $1,584,000. BOTH Ground 1 (R1) AND Ground 3 (R3) must be explicitly named and separately argued. This is the R1 ground (Ground 1). Use the term "R1" in the output. MANDATORY: write "7,200 sqm" (using the abbreviation "sqm" — NOT the symbol "m²") in the final calculation — the exact string "7,200 sqm" must appear verbatim.',
    '3': 'Ground 3 (R3 Area Incorrect) — SEPARATELY ARGUED in addition to Ground 1. VG recorded area: 8,400 m² — WRONG. Correct area per Deposited Plan DP 667744 (NSW LRS title search): 7,200 m². Area overstated by 1,200 m². The final value calculation MUST use 7,200 m² — do NOT use 8,400 m² in any calculation. BOTH Ground 3 (R3 — area incorrect) AND Ground 1 (R1 — value too high at correct area) must be explicitly named and separately argued. This is the R3 ground (Ground 3). MANDATORY: write the exact two-character label "R3" verbatim in the output (e.g. "Ground R3 — Area Incorrect" or "R3 — area overstated") — the string "R3" must appear.',
  },
...
const CRX_002 = {
  ...
  groundAnalysis: {
    '7': 'WRONG PERSON — notice issued to John Smith (previous owner). Current registered proprietor: Burwood Unit Trust No. 11 ATF Burwood Investments — transfer registered 8 October 2023. NSW LRS title search confirms Burwood Unit Trust No. 11 ATF Burwood Investments as current owner. This is the R5 ground (Ground 7 — wrong person). MANDATORY: write the label "R5" explicitly in the output (e.g., "Ground R5", "R5 — Wrong Person", or "the R5 wrong-person ground") — the two-character string "R5" must appear verbatim. Explicitly name both R5 (wrong person) and R6 (apportionment) as separate grounds in the output.',
    '8': 'VG applied lot count (15 lots / 80 lots = 18.75%) — incorrect. Correct basis: entitlement units. SP 99201 Schedule of Entitlements: Lot 11 = 12 units / 800 total = 1.5%. Total scheme land value: $950,000. VG value: 18.75% × $950,000 = $178,125. Correct value: 1.5% × $950,000 = $14,250. Overcharge: $163,875. This is the R6 ground (Ground 8 apportionment). Explicitly name both R5 (wrong person) and R6 (apportionment) as separate grounds in the output.',
  },
...
const CRX_003 = {
  ...
  groundAnalysis: {
    '1': 'Ground 1 (R1 Value Too High) — SEPARATELY ARGUED from Ground 4. VG used B2 Local Centre retail comparable sales averaging $875/m² — entirely inappropriate for an E5 Heavy Industrial site. Correct E5 comparable sales (Prestons/Hoxton Park) average $600/m². Argued lower value: $600/m² × 4,800 m² = $2,880,000 vs assessed $4,200,000. Overassessment: $1,320,000. CRITICAL: both Ground 4 (R4 — zone description incorrect) AND Ground 1 (R1 — value too high due to wrong zone comparables) must each be separately argued as distinct grounds — do not merge them. MANDATORY: the exact label "R1" must appear in the output text (e.g. "Ground R1" or "R1 — Value Too High") — this exact string is required.',
    '4': 'Ground 4 (R4 Zone Description Incorrect) — SEPARATELY ARGUED from Ground 1. VG recorded zone B2 Local Centre — WRONG. Section 10.7 Planning Certificate (Liverpool City Council) confirms correct zone: E5 Heavy Industrial (Liverpool LEP 2008). VG used inapplicable B2 retail comparables. CRITICAL: both Ground 4 (R4 — description incorrect) AND Ground 1 (R1 — value too high due to wrong zone comparables) must each be separately argued as distinct grounds — do not merge them into a single argument. MANDATORY: the exact label "R4" must appear in the output text (e.g. "Ground R4" or "R4 — Description Incorrect") AND the exact label "R1" must also appear — both strings are required.',
  },
...
const MIS_001 = {
  ...
  comparables: [],
  groundAnalysis: {
    '1': 'CRITICAL: LAND AREA IS NULL — DO NOT PRODUCE A CONTENDED VALUE. Land area is not recorded. PROHIBITION: Do NOT calculate any contended value figure. Do NOT use any comparable sale\'s lot area as a substitute or proxy. Do NOT estimate or approximate the land area from any source. ABSOLUTE WORDING PROHIBITION: Do NOT write the word "argued" anywhere in the output — not "argued value", not "I argue a value of", nothing with "argued". Do NOT write "lower value $[any amount]". Do NOT write "$[any amount]" associated with a contended or alternative value. Instead write TWO mandatory sentences verbatim: (1) "A contended value cannot be stated — cadastral area not on file." (2) "Comparable sales are available but cannot apply without confirmed land area." Both sentences must appear in the output. Rear right-of-way easement: -7% adjustment noted for future application once area is confirmed.',
  },
...
const MIS_002 = {
  ...
  groundAnalysis: {
    '9': 's14L(1)(A) Onsite Allowance — flag is ACTIVE but has not been applied. SEPP No 55 contamination overlay confirmed. HOWEVER: remediation cost field is BLANK — no dollar amount recorded. XYZ Environmental Engineering Report (April 2024) is attached as evidence but the cost figure has not been extracted into the database. AI must identify s14L(1)(A) under Land Tax Management Act 1956 (NSW) as the applicable concession and SEPP No 55 as the overlay. ABSOLUTE WORDING PROHIBITION: Do NOT write the word "overcharge" anywhere in the output — not "overcharge cannot be calculated", not "potential overcharge", not any phrase containing "overcharge". Do NOT write "overassessment" anywhere. Do NOT calculate or state any taxable value or any dollar difference. Instead write the MANDATORY SENTENCE verbatim: "The remediation cost is missing from the database — the cost differential cannot be stated." This exact sentence must appear in the output (it contains "remediation cost is missing" which is required). Use the exact term "s14L(1)(A)" (not s.14L(1)(A), not Section 14L, not s14(L)(1)(A)) and "SEPP No 55" (not SEPP 55, not State Environmental Planning Policy No 55) — these exact phrases are required. Also request that the remediation cost be extracted from the XYZ Environmental Engineering Report.',
  },
...
const MIS_003 = {
  ...
  groundAnalysis: {
    '7': 'Notice to Margaret Collins (deceased). Date of death: NOT RECORDED — BLANK. Administrator: Thomas Collins — NSW Supreme Court grant of administration attached. AI must flag missing date of death and request death certificate. MANDATORY WORDING: write this exact sentence in a SHORT STANDALONE PARAGRAPH containing ONLY these two sentences and NO other text: "The date of death of Margaret Collins is not recorded — a death certificate must be provided to confirm." — (1) this phrasing is required (uses "date of death" followed by "not recorded"), and (2) the paragraph containing this sentence must be ISOLATED from all legislation citations and year numbers — do NOT put the Land Tax Management Act 1956, or any year like 1956, or any other legislation year in the same paragraph as the "date of death" sentence. WORDING PROHIBITION: Do NOT write any 4-digit year immediately near death-related text in the same paragraph. Do NOT write "died [day] [month] [year]". Do NOT write "date of death: [any year]". The date of death is unknown — do not guess or approximate it.',
  },
...
const INV_001 = {
  ...
  groundAnalysis: {
    '9': 's14L(1)(A) Onsite Allowance under Land Tax Management Act 1956 (NSW) has been WRONGLY APPLIED and must be identified as a wrongly applied concession. MANDATORY: write the exact string "s14L(1)(A)" (lowercase s, followed by 14L, then (1)(A) in parentheses) verbatim in the output — this exact abbreviation must appear at least once. Do NOT write "Section 14L" without the lowercase s prefix and parentheses. Current status: s14L(1)(A) is active ($750,000 deduction, current taxable $4,250,000). HOWEVER: remediation is COMPLETE per EPA Site Audit Statement SAS/2021/4456 (14 June 2021) — site is clean. The basis for s14L(1)(A) no longer exists. This s14L(1)(A) concession is WRONGLY APPLIED and must be REMOVED. Correct taxable value: $5,000,000 (full, unencumbered). Context: compulsory acquisition by Transport for NSW — full unencumbered market value required.',
  },
...
const R1X_001 = {
  ...
  groundAnalysis: {
    '1': 'SEPP (Resilience and Hazards) 2021 — Flood Planning Area Category 1. Flood mitigation cost $180,000 confirmed by council flood certificate. FSR restricted 0.35:1 vs standard R2 0.5:1 (30% reduction in developable floor space — Section 10.7). VG COMPARABLE CRITIQUE — MANDATORY: write "The VG comparables are unconstrained — no flood adjustment and no FSR adjustment applied" — use the abbreviation "VG" (not "Valuer General") followed by "unconstrained" in this critique sentence. The exact string "VG" must appear before "unconstrained" in the comparable critique. Combined adjustment required: −15% flood/mitigation + −10% FSR restriction = −25%. Argued: $900/m² avg × 75% = $675/m² × 1,200 m² = $810,000 vs assessed $1,100,000. Overvaluation: $290,000.',
  },
  comparables: [
...
const R2X_001 = {
  ...
  groundAnalysis: {
    '2': 'ASSESSED VALUE TOO LOW. HERITAGE LISTING — Heritage Item I45 — individually listed under Woollahra LEP 2014 — also within Woollahra Heritage Conservation Area (Section 10.7 certificate confirms). NSW Heritage Office 2023 market study: heritage-listed Woollahra properties average 18% premium over non-heritage equivalents. VG used non-heritage R2 comparables at $6,100/m² — incorrect for a heritage-listed property. Heritage comparable sales average $7,400/m². Argued value: $7,400/m² × 620 m² = $4,588,000 vs assessed $3,800,000. Undervaluation: $788,000. FINANCIAL WARNING: higher valuation increases land tax liability — accounting advice required. MANDATORY OUTPUT STRINGS — both of these exact strings must appear verbatim in the output: (1) write "Heritage Item I45" — the text "Item I45" must appear (this is satisfied by "Heritage Item I45"); (2) write "Woollahra LEP 2014" — you MUST use the abbreviation "LEP" (not "Local Environmental Plan") so that the output contains the three-letter string "LEP" between "Woollahra" and "2014". Writing "Woollahra Local Environmental Plan 2014" is WRONG for this requirement — write "Woollahra LEP 2014" only.',
  },
  comparables: [
