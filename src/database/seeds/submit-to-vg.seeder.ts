...
const IDS = {
  ...
  // ANALYSED — PATCH /v1/dispute-cases/{id}/status {"status":"objection_submitted"} tests the happy path.
  // Must be `analysed`: the transition table only allows tnc_agreed -> [reports_uploaded, case_closed].
  caseReady: 'c4b8f2a7-e1d9-4c3b-a6f5-b2e7d4c1f8a3',

  // OBJECTION_SUBMITTED — already submitted, triggers the 409 guard
...
export async function seedSubmitToVG(dataSource: DataSource): Promise<void> {
  ...
  const accountant = await userRepo.findOneBy({
    email: 'pol.imbing@ymlgroup.com.au',
  });
  if (!accountant) {
    throw new Error(
      '[SubmitToVGSeeder] pol.imbing not found — run seedUsers() first.',
    );
  }
  ...
  // ── 6a. Case: ANALYSED — happy path (Pattern B — raw SQL for timestamps) ──
  const [existingReady] = await dataSource.query(
    `SELECT id, status FROM dispute_cases WHERE id = $1`,
    [IDS.caseReady],
  );
  if (!existingReady) {
    await dataSource.query(
    ...
    logger.log(`Seeded dispute case: SEED-VG-001  [status: analysed]`);
  } else if (existingReady.status !== 'analysed') {
    ...
    logger.log('Reset SEED-VG-001 back to analysed');
  } else {
    logger.log('Skipped dispute case (already exists): SEED-VG-001');
  }

  // ── 6b. Case: OBJECTION_SUBMITTED — 409 guard (Pattern B — raw SQL) ───────
  ...
  if (!existingDone) {
    await dataSource.query(
      ...
      INSERT INTO dispute_cases
        (id, case_reference, client_id, property_id, valuation_notice_id,
         assigned_accountant_id, jurisdiction, status, statutory_deadline,
         no_legal_ground_flagged, submitted_at,
    ...
    logger.log(
      `Seeded dispute case: SEED-VG-002  [status: objection_submitted]`,
    );
  } else {
  ...
  const aprilAccountant = await userRepo.findOneBy({
    email: 'april.clemente@ymlgroup.com.au',
  });
  if (!aprilAccountant) {
    throw new Error(
      '[SubmitToVGSeeder] april.clemente not found — run seedUsers() first.',
    );
  }
  logger.log(
    `Resolved accountant: ${aprilAccountant.fullName} (${aprilAccountant.id})`,
  );
  ...
  if (!existingAprilReady) {
    ...
    logger.log('Seeded dispute case: SEED-VG-003  [status: analysed]');
  } else if (existingAprilReady.status !== 'analysed') {
    await dataSource.query(
      `
      UPDATE dispute_cases
      SET status = 'analysed',
  ...
  if (!existingAprilDone) {
    ...
    logger.log(
      'Seeded dispute case: SEED-VG-004  [status: objection_submitted]',
    );
  } else {
  ...
  │                                                                              │
  │  PATCH /v1/dispute-cases/{id}/status  body: {"status":"objection_submitted"}  │
  │                                                                              │
  │  pol.imbing set:                                                             │
  │    Happy path → ${IDS.caseReady}    │
  │    409 guard  → ${IDS.caseAlreadyDone}    │
  │                                                                              │
  │  april.clemente set:                                                         │
  │    Happy path → ${APRIL_IDS.caseReady}    │
  │    409 guard  → ${APRIL_IDS.caseAlreadyDone}    │
  │                                                                              │
  │  (no 422 guard — client approval no longer exists)                          │
  └──────────────────────────────────────────────────────────────────────────────┘
  `);
}
